...
const EXCLUDE_DIRS = new Set(['.git','node_modules','android','nur_makkah','hajj_app_flutter','archive','build']);
const FILE_EXT_WHITELIST = new Set(['.js','.ts','.tsx','.json','.md','.yaml','.yml','.txt','.html','.css','.scss','.sh']);
...
(async function main(){
  console.log('Running safe rename: nur_makkah -> nur_makkah (excluding android/ and native packages)');
  await walk(ROOT);
