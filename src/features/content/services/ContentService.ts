...
const PILLAR_DESC_MAP_AR: Record<string, string> = {
  ...
  "Agjërimi i Ramazanit":
    "الإمساك عن الطعام والشراب والشهوة من الفجر إلى الغروب في شهر رمضان",
...
const PILLAR_DESC_MAP_EN: Record<string, string> = {
  ...
  "Falja e namazit":
    "Performing the five obligatory prayers daily and nightly.",
  "Dhënja e Zekatit": "Giving a portion of wealth to the needy and poor.",
  "Agjërimi i Ramazanit":
    "Abstaining from food, drink, and desires from dawn to sunset in Ramadan.",
  "Haxhi në Qabe":
    "Visiting the Holy Kaaba at least once in a lifetime for those who are able.",
};
...
export class ContentService implements DataServiceInterface {
  ...
  private miqatCoordinatesCache = new Map<
    string,
  ...
  private async loadHajjData(): Promise<void> {
    ...
    try {
      ...
      const storedData = await AsyncStorage.getItem(
        APP_CONFIG.storage.hajjDataKey,
      );
      if (storedData) {
      ...
      console.warn(
        "Failed to load data from storage, using legacy data:",
        error,
      );
    }
      ...
      const transformed: HajjData = {
        ...
        introduction: {
          description: {
            ...
            arabic:
              "الحج هو أحد أركان الإسلام الخمسة، وهو فرض على كل مسلم قادر على أدائه مرة واحدة في العمر على الأقل",
            english:
              "Hajj is one of the five pillars of Islam and is obligatory for every Muslim who is able to perform it at least once in a lifetime",
            quranic_reference:
          ...
          qabja: {
            ...
            arabic:
              "الكعبة هي المكان الذي تجذب إليه قلوب المسلمين من جميع أنحاء العالم، وهي أول بيت بني للعبادة على الأرض",
            english:
              "The Kaaba is the place that attracts the hearts of Muslims from all over the world, and it is the first house built for worship on earth",
            quranic_reference:
              "إِنَّ أَوَّلَ بَيْتٍ وُضِعَ لِلنَّاسِ لَلَّذِي بِبَكَّةَ مُبَارَكًا وَهُدًى لِّلْعَٰلَمِينَ",
          },
        },
        shtyllat_e_islamit: this.transformPillars(
          legacyData.shtyllat_e_islamit || [],
        ),
        detyrimi_i_haxhit: this.transformHajjObligation(
          legacyData.detyrimi_i_haxhit || {},
        ),
        edukata_e_udhetimit: this.transformTravelEtiquette(
          legacyData.edukata_e_udhetimit || [],
        ),
        ihrami: this.transformIhramInfo(legacyData.ihrami || {}),
        ndalesat_gjate_ihramit: this.transformProhibitions(
          legacyData.ndalesat_gjate_ihramit || [],
        ),
        ...
        qabja: {
          ...
          rendesia: {
            ...
            arabic:
              "يتوجه المسلمون نحو الكعبة خمس مرات في اليوم لأداء الصلاة ويطوفون حولها لأداء مناسك الحج",
            english:
              "Muslims face the Kaaba five times a day for prayer and circumambulate it during Hajj rites",
          },
        },
        rendesia_e_haxhit: {
          description: {
            ...
            arabic:
              "الحج هو أحد أركان الإسلام وهو فرض على كل مسلم قادر على أدائه",
            english:
              "Hajj is one of the pillars of Islam and is obligatory for every Muslim who is able to perform it",
          },
          hadith: {
            albanian: legacyData.rendesia_e_haxhit?.hadith || "",
            arabic: "الحج فرض مرة واحدة في العمر، ومن زاد فهو نافلة",
            english:
              "Hajj is obligatory once in a lifetime, and whoever does more than that is voluntary",
          },
          keshilla: {
            albanian: legacyData.rendesia_e_haxhit?.keshilla || "",
            arabic:
              "لمزيد من التفاصيل والإرشادات المحددة، يوصى بالاستشارة بمصادر متخصصة وعلماء إسلاميين",
            english:
              "For more details and specific guidance, it is recommended to consult specialized sources and Islamic scholars",
          },
        },
      };
  ...
  private transformHajjObligation(legacyObligation: any) {
    return {
      ...
      hadith: {
        ...
        english:
          "Hajj is once in a lifetime. Whoever does more than that is voluntary.",
        reference: "Sahih al-Bukhari, Book 26, Hadith 1513",
        full_arabic_hadith:
          "عن أبي هريرة رضي الله عنه قال: خطبنا رسول الله صلى الله عليه وسلم فقال: يا أيها الناس، قد فرض الله عليكم الحج فحجوا. فقال رجل: أكل عام يا رسول الله؟ فسكت، حتى قالها ثلاثا. فقال رسول الله صلى الله عليه وسلم: لو قلت نعم لوجبت، ولما استطعتم. الحج مرة واحدة، فمن زاد فهو نافلة",
      },
      kushtet: {
        albanian: legacyObligation.kushtet,
        arabic:
          "الحج يجب أن يكون خالصاً لله، ويؤدى على سنة رسول الله صلى الله عليه وسلم",
        english:
          "Hajj must be purely for Allah and performed according to the Sunnah of the Messenger of Allah",
        quranic_reference:
  ...
  private transformIhramInfo(legacyIhram: any) {
    return {
      description: {
        ...
        english:
          "Ihram is the intention of a Muslim to perform the rites of Hajj or Umrah",
      },
      koha: {
        albanian: legacyIhram.koha,
        arabic: "نية الحج تكون في أشهر الحج: شوال، وذو القعدة، وذو الحجة",
        english:
          "The intention for Hajj is made in the months of Hajj: Shawwal, Dhul-Qa'dah, and Dhul-Hijjah",
        quranic_reference: "ٱلْحَجُّ أَشْهُرٌ مَّعْلُومَٰتٌ",
      },
      llojet_e_nijetit: (legacyIhram.llojet_e_nijetit || []).map(
        (item: any) => ({
          lloji: {
            albanian: item.lloji,
      ...
      talbiyyah_complete: {
        ...
        english_translation:
          "Here I am at Your service, O Allah, here I am at Your service. Here I am at Your service, You have no partner, here I am at Your service. Verily all praise and blessings are Yours, and all sovereignty. You have no partner.",
        hadith_reference: "Sahih al-Bukhari, Book 26, Hadith 1549",
      },
      para_veshjes: (legacyIhram.para_veshjes || []).map(
  ...
  private transformProhibitions(legacyProhibitions: any[]) {
    return legacyProhibitions.map((item, index) => ({
      ...
      evidence: {
        quranic:
          "يَا أَيُّهَا ٱلَّذِينَ ءَامَنُوا۟ لَا تُحِلُّوا۟ شَيْـَٰٓئَاتِ ٱلصَّيْدِ وَأَنتُمْ حُرُمٌ",
  ...
  private getPillarArabicName(name: string): string {
    if (this.pillarArabicCache.has(name))
      return this.pillarArabicCache.get(name)!;
  ...
  private getPillarEnglishName(name: string): string {
    if (this.pillarEnglishCache.has(name))
      return this.pillarEnglishCache.get(name)!;
  ...
  private getPillarArabicDescription(name: string): string {
    if (this.pillarArabicDescCache.has(name))
      return this.pillarArabicDescCache.get(name)!;
  ...
  private getPillarEnglishDescription(name: string): string {
    if (this.pillarEnglishDescCache.has(name))
      return this.pillarEnglishDescCache.get(name)!;
    ...
    switch (categoryName) {
      case CATEGORIES.PILLARS_OF_ISLAM:
        rules = this.hajjData!.shtyllat_e_islamit.map((pillar, index) => ({
          ...
          order: pillar.order,
          evidence: {
            quranic: [pillar.description.quranic_reference].filter(
              Boolean,
            ) as string[],
            hadith:
              pillar.description && (pillar.description as any).hadith_reference
                ? [(pillar.description as any).hadith_reference]
                : [],
          },
        }));
        break;
      ...
      case CATEGORIES.IHRAM_PROHIBITIONS:
        rules = this.hajjData!.ndalesat_gjate_ihramit.map(
          (prohibition, index) => ({
            ...
            evidence: prohibition.evidence
              ? {
                  quranic: prohibition.evidence.quranic
                    ? [prohibition.evidence.quranic]
                    : [],
                  hadith: prohibition.evidence.hadith
                    ? [prohibition.evidence.hadith]
                    : [],
                }
              : undefined,
          }),
